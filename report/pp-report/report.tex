...
This dissertation performs a very thorough examination of performance for existing codes, but, unlike \cite{dis1}, makes no attempt to improve performance. I does, however justify well what the reasons for the performances of the varying codes that are tested are. It also gives suggestions as to how these codes could be improved.
...
\center
  \begin{tabular}{| l | c | r |}
    \hline
    Task & Start Date & Duration \\ \hline
    ...
    Write up - including presentation and poster & 27/07/2017 & 3 weeks \\ \hline 
    
  \end{tabular}
  \caption{Table showing formal plan.}
  \label{plan}
...
Below follows a more formal risk analysis, where more specific risks are given along with the likelihood and what will be done to mitigate them.

\subsection{Risk 1: Work delayed by bug}
\textbf{Likelihood: Medium} 

\textbf{Impact: Medium} 

It is possible that a bug, even a very simple one, could prevent the code from working and potentially take weeks to fix.

\textbf{Mitigation:} To prevent this happening the code will be thoroughly unit tested from the start. In addition, if it does happen there will be less time available to investigate and carry out possible extensions. While this is not ideal, it will still be possible to produce a credible dissertation.

\subsection{Risk 2: Unable to gain access to AAG machine}
\textbf{Likelihood: Minimal}
...
\textbf{Mitigation:} It will still be possible to undertake some experiments on a home desktop equipped with a GTX 970 GPU. In addition to this it would be possible to use cloud servers equipped with GPUs from a provider such as Amazon Web Services.

\subsection{Risk 3: Supervisor unavailable}
\textbf{Likelihood: Minimal}
...
\textbf{Mitigation:} There is very little that can be done to mitigate for this. However, documenting all work done and keeping an updated wiki can give them the necessary information and help to show that effort has consistently been put in.
...
\begin{lstlisting}[float,caption=First 2D example]
...
\bibitem{dis1}
Capeli, L. (2016) \emph{Fulfilling Solvency II regulations using GPUs.} MSc Thesis. University of Edinburgh. Available at: \url{https://static.ph.ed.ac.uk/dissertations/hpc-msc/2015-2016/Ludovic_Capelli-Dissertation.pdf}

\bibitem{dis2}
Curry, C. (2016) \emph{Assessing the Performance of Optimised Primality Tests.} MSc Thesis. University of Edinburgh. Available at: \url{https://static.ph.ed.ac.uk/dissertations/hpc-msc/2015-2016/Cameron_Curry-HPC_Dissertation.pdf}
