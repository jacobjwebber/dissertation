...
The example chosen to determine the efficiency of memory usage was that of a rotating unit square wholly contained by a bounding square. This arrangement is shown in figure \ref{square}. Varying $\theta$ also varies the percentage of the space not contained within the unit square. When $\theta$ - the angle of rotation of the internal square - is zero, a simple 2D array is perfectly efficient, storing no out of bounds points. This perfect efficiency is not mirrored by the array of structs in this case, as there is additional overhead of storing the location of the neighboring points. When $\theta$ is not zero, inefficiencies are introduced by storing values in the bounding square that are not in the internal square. There is useful work to be done in plotting how memory efficiencies change with both $\theta$ and the size of the sub-arrays.
